int LUI_EXP l_uiNewFontDescriptor(lua_State *L)
{
...
int LUI_EXP l_uiFontDescriptorFammily(lua_State *L)
{
...
int LUI_EXP l_uiFontDescriptorSize(lua_State *L)
{
  uiFontDescriptor *desc = CHECK_USER_OBJECT(1, FontDescriptor);
...
int LUI_EXP l_uiFontDescriptorWeight(lua_State *L)
{
  uiFontDescriptor *desc = CHECK_USER_OBJECT(1, FontDescriptor);
...
int LUI_EXP l_uiFontDescriptorItalic(lua_State *L)
{
  uiFontDescriptor *desc = CHECK_USER_OBJECT(1, FontDescriptor);
...
int LUI_EXP l_uiFontDescriptorStretch(lua_State *L)
{
  uiFontDescriptor *desc = CHECK_USER_OBJECT(1, FontDescriptor);
...
int LUI_EXP l_uiFreeFontButtonFont(lua_State *L)
{
  uiFontDescriptor *desc = CHECK_USER_OBJECT(1, FontDescriptor);
...
int LUI_EXP l_uiNewFontButton(lua_State *L)
{
  UI_CREATE_OBJECT(FontButton, uiNewFontButton());
  return 1;
}

int LUI_EXP l_uiFontButtonFont(lua_State *L)
{
...
void LUI_EXP on_fontbutton_changed(uiFontButton *b, void *data)
{
  callback(data, b);
}

int LUI_EXP l_uiFontButtonOnChanged(lua_State *L)
{
