...
if not _ then print('not support thread here') end
...
local function onClosing(w, data)
  if timeThread then timeThread:join() end
  ui.Quit()
  return true
end
...
function uiMain(void)
  ...
  w = ui.NewWindow("Hello", 320, 240, false):Margined(true):SetChild(b)

  e = ui.NewMultilineEntry():ReadOnly(true)

  btn = ui.NewButton("Say Something"):OnClicked(saySomething)
  b:Append(btn, false):Append(e, true)
